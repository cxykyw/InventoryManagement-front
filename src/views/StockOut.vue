      ...
      <div class="search-area">
        <el-form :inline="true" :model="searchForm" class="demo-form-inline">
          ...
          <el-form-item label="出库类型">
            <el-select v-model="searchForm.type" placeholder="请选择出库类型">
    ...
    >
      ...
      >
        ...
        <el-form-item label="客户" prop="customer">
          <el-select v-model="formData.customer" placeholder="请选择客户">
            <el-option
...
import { mockApi, warehouses, customers, products } from '../mock/data'
...
const handleAudit = () => {
  ...
  ElMessageBox.confirm('确定要审核选中的单据吗？', '提示', {
    ...
    for (const row of selectedRows.value) {
      await mockApi.auditDocument(row.id, 'out')
    }
...
const submitForm = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      await mockApi.addStockOut(data)
      ElMessage.success(dialogTitle.value === '新增出库单' ? '新增成功' : '编辑成功')
